...
In game: `/ramble`, `/ramble status`, `/ramble note <text>`, `/ramble mark`, `/ramble end`, `/ramble chapters`,
`/ramble debug [on|off]`, `/ramble dump`.
...
`ramble export latest`, `ramble summarize latest [--no-ai]`, `ramble page latest` (HTML story page),
